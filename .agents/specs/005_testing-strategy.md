...
- `buildinfo.lua` — `clean` extracts the numeric-keyed app block from a dump with a command echo, missing newlines, or trailing noise and returns an empty string for an echo-only dump, `parse` reads `buildid` and the `InstalledDepots` manifests, `validate` rejects a dump that lacks a required field, and `merge` takes `buildid` from the base entry and returns the union of the depot maps.
...
Integration tests create a temporary Steam root, seed it with fixture `libraryfolders.vdf` and `appmanifest_*.acf` files, set `MILLENNIUM__STEAM_PATH`, and run the operations through the backend. They assert that lock writes the update-state invariant, including the merged base and DLC depot manifests, and stores the verbatim `original`; that refresh updates `locked_build` and `refreshed_at`; that unlock restores the `original` text and deletes the record; that reapply repairs an appmanifest Steam rewrote; that discovery finds the appmanifest after the install directory moves; that migration rolls back on a failure; that each operation emits the record [Spec 4](004_app-version-lock.md#logging) fixes; and that Restore All restores the healthy apps, keeps the failed records, and drops the uninstalled records.

### Contract Tests

Contract tests assert the thirteen frontend-to-backend method names and the one backend-to-frontend method name from [Spec 4](004_app-version-lock.md#bridge) and [Spec 6](006_logging.md#relay-bridge), and the payload and response shape of each against the shared TypeScript types and matching Lua shape assertions. They cover the `get_required_apps` method and the `dumps` map of `lock_app` and `refresh_app`. The `append_log` method and its payload come from [Spec 6](006_logging.md#relay-bridge). They also assert that the two catalogs carry an identical key set and that every code in [Spec 8](008_localization.md#backend-error-codes) has an `error.<code>` key, with no orphaned `error.*` key.
