...
- Unit tests cover the pure backend logic — the `vdf.lua` codec, the `buildinfo.lua` parser, the `acf.lua` appmanifest writer, the `state.lua` record store, the `paths.lua` resolver and discovery, the `migrate.lua` migration, the `lock.lua` operations, the `clock.lua` clock reader, and the `log.lua` logging module — and the frontend logic in `console.ts`, `watch.ts`, `locked.ts`, `log.ts`, `i18n.ts`, `properties.tsx`, `gamepage.tsx`, and `time.ts`. The tests inject a fake filesystem, a fake clock, a fake `SteamClient`, and a fake `Millennium`.
...
- `buildinfo.lua`  
  `clean` extracts the numeric-keyed app block from a dump with a command echo, missing newlines, or trailing noise and returns an empty string for an echo-only dump, `parse` reads `buildid` and the `InstalledDepots` manifests, `validate` rejects a dump that lacks a required field, and `merge` takes `buildid` from the base entry and returns the union of the depot maps.
- `acf.lua`  
  `read` parses an appmanifest, `set` writes one field, and `write` goes through a temporary file and a rename; a failed write leaves the previous file intact.
- `state.lua`  
  `path`, `write`, `read`, `list`, and `remove` manage one record per file, and a record whose `version` is not `1` is refused on load.
- `paths.lua`  
  `resolve` applies the `data_root` precedence and reports `is_default`; `defaults` returns the OS-conventional directory; `validate` rejects a path that is not absolute, that is equal to or nests with the current data root, or that cannot be created or written; `find_appmanifest` unions `steamapps/libraryfolders.vdf`, `config/libraryfolders.vdf`, and the Steam root; `resolve_manifest` falls back to discovery when the cached path is stale.
- `migrate.lua`  
  `move` copies, verifies, persists, and then deletes; a failure before the new path is persisted leaves the old root intact and removes the partial copy.
- `clock.lua`  
  `is_24h` reads `b24HourClock` from the roaming `sharedconfig.vdf`, reports the 12-hour value as `false`, falls back to `UserLocalConfigStore` in `localconfig.vdf`, and returns `nil` when no config carries the setting or when `MILLENNIUM__STEAM_PATH` is absent.
- `lock.lua`  
...
- `console.ts`  
  `capture_build_info` runs `app_info_print`, samples the spew until the app block appears, and returns the dump; a dump that carries only the command echo, a dump with no `depots` table, and an empty response each fail the capture, and the command builder rejects a non-numeric `appid`; `capture_build_info_set` captures the base app, calls `get_required_apps`, captures each returned DLC app in order, and returns the dumps keyed by app id, and it fails the set when a required capture fails; `capture_then_refresh` passes the dumps to `refresh_app` and refreshes only after a successful set; an accepted capture relays an `info` record, and a failed capture relays an `error` record.
- `errors.ts`  
...
- `locked.ts`  
  `sync_locked_ids` replaces the local locked set from a record list, treats a `{}` response as an empty list, and keeps the set for an error envelope.
- `i18n.ts`  
  `init_i18n` selects the catalog for the client language, resolves English for an unknown language or a rejected call, and does nothing on a later call; `refresh_locale` picks up a changed language and keeps the current catalog on a rejected or unknown result; `t` interpolates a parameter, keeps a placeholder without one, ignores a parameter without a placeholder, and returns the key for a missing key; `resolve_error` localizes a known code, falls back to the `error` text, then to `error.unknown`; `set_locale` resolves an unknown name to English.
- `properties.tsx`  
  `format_time` renders a timestamp through the shared `time.ts` formatter and returns `null` for a missing or zero value, `format_lock_text` pretty-prints a JSON object at two spaces of indentation in the record's schema order — known fields first and an unknown field appended — and returns any other text, including an array, unchanged, `behavior_label` names the known `EAppAutoUpdateBehavior` values and falls back for an unknown or absent one, and `find_record` selects the record for one app id regardless of its type; the tab's DOM injection stays on the manual checklist.
- `gamepage.tsx`  
  `appid_from_path` reads a numeric app id from a library path and rejects a path without one, `appid_from_image_src` reads the app id from a `library_hero` image URL and rejects a URL without one, and `merge_style` merges a sampled style over the fallback field by field; the badge's DOM injection stays on the manual checklist.
- `time.ts`  
  `format_client_time` returns `undefined` for a missing or non-positive value, includes the year by default, omits the current year when `current_year_short` is set, follows the `hour12` flag, and falls back to `en` for an invalid locale.
- `log.ts`  
  `log_info`, `log_warn`, and `log_error` write the matching console method with the `[steamapp-verlock]` prefix and relay one `{ level, message }` record through `bridge.append_log`; a rejected relay is swallowed and the console line still appears.

### Integration Tests

Integration tests create a temporary Steam root, seed it with fixture `libraryfolders.vdf` and `appmanifest_*.acf` files, set `MILLENNIUM__STEAM_PATH`, and run the operations through the backend.

They assert that:

- `Lock` writes the update-state invariant, including the merged base and DLC depot manifests, and stores the verbatim `original`;
- `Refresh` updates `locked_build` and `refreshed_at`;
- `Unlock` restores the `original` text and deletes the record;
- `Reapply` repairs an appmanifest Steam rewrote;
- discovery finds the appmanifest after the install directory moves;
- migration rolls back on a failure;
- each operation emits the record [Spec 4](004_app-version-lock.md#logging) fixes;
- `Restore All` restores the healthy apps, keeps the failed records, and drops the uninstalled records.

### Contract Tests

Contract tests assert the thirteen frontend-to-backend method names and the one backend-to-frontend method name from [Spec 4](004_app-version-lock.md#bridge) and [Spec 6](006_logging.md#relay-bridge), and the payload and response shape of each against the shared TypeScript types and matching Lua shape assertions.

They cover the `get_required_apps` method and the `dumps` map of `lock_app` and `refresh_app`. The `append_log` method and its payload come from [Spec 6](006_logging.md#relay-bridge).

They also assert that the two catalogs carry an identical key set and that every code in [Spec 8](008_localization.md#backend-error-codes) has an `error.<code>` key, with no orphaned `error.*` key.

### Test Doubles and Seams

The backend tests replace the filesystem module in `package.loaded` before they require the module under test, so the production functions keep the signatures [Spec 4](004_app-version-lock.md#backend-functions) fixes and gain no test-only parameter. The tests save and restore `os.time` for the clock.

The frontend tests set `globalThis.SteamClient` to a fake with the `Console` and `Apps` methods and use the Bun fake timers. Every frontend test file registers the same `millennium` double from `frontend/tests/millennium_mock.ts`, because Bun links a mocked module once per test run and a per-file double would leave a later file with a missing export.

The backend tests stub the `Millennium` global for the frontend calls and the config API. The logging tests include `log` in `BACKEND_MODULES` and assert against the calls the `logger` stub records. Integration tests use a real temporary directory.

### Console Capture Tests

A fake `SteamClient.Console` feeds recorded `app_info_print` dumps to the capture module. The fixtures include a multi-depot dump and a beta-branch dump for the backend parser, and the frontend capture test emits a valid dump, an echo-only response, and no response.
...
The fixtures live under `backend/tests/fixtures/` and `frontend/tests/fixtures/`. The backend fixtures carry `libraryfolders.vdf`, single-depot, multi-depot, pending-state, and multi-OS `appmanifest_*.acf` files, and `app_info_print` dumps. The frontend fixtures carry the recorded console spew for each dump scenario.
