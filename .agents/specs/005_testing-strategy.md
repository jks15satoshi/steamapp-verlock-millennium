...
- `properties.tsx` — `format_time` renders a timestamp through the shared `time.ts` formatter and returns `null` for a missing or zero value, `format_lock_text` pretty-prints a JSON object at two spaces of indentation and returns any other text unchanged, `behavior_label` names the known `EAppAutoUpdateBehavior` values and falls back for an unknown or absent one, and `find_record` selects the record for one app id regardless of its type; the tab's DOM injection stays on the manual checklist.
