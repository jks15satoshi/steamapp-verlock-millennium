...
describe("paths", function()
    ...
    describe("defaults", function()
        it("returns the XDG-conventional directories", function()
            ...
            assert.equals("/xdg/data/steamapp-verlock", strip(roots.data_root))
            assert.equals("/xdg/cache/steamapp-verlock", strip(roots.cache_root))
        end)
    end)
    ...
    describe("resolve", function()
        ...
        it("falls back to MILLENNIUM__CONFIG_PATH", function()
            ...
            support.hide_env("HOME")
            support.set_env("MILLENNIUM__CONFIG_PATH", "/cfg")
            local paths = load_paths()
            local roots = paths.resolve()
            assert.equals("/cfg/steamapp-verlock", strip(roots.data_root))
        end)

        it("falls back to the config path for the cache root when a data_root is configured", function()
            support.set_env("XDG_DATA_HOME", "/xdg/data")
            support.hide_env("XDG_CACHE_HOME")
            support.hide_env("HOME")
            support.set_env("MILLENNIUM__CONFIG_PATH", "/cfg")
            ...
            local roots = paths.resolve()
            assert.equals("/custom/verlock", strip(roots.data_root))
            assert.equals("/cfg/steamapp-verlock/cache", strip(roots.cache_root))
        end)

        it("falls back to the config path for the cache root when only the data home variable exists", function()
            support.set_env("XDG_DATA_HOME", "/xdg/data")
            support.hide_env("XDG_CACHE_HOME")
            support.hide_env("HOME")
            support.set_env("MILLENNIUM__CONFIG_PATH", "/cfg")
    ...
    describe("validate", function()
        ...
        it("accepts a distinct absolute path", function()
            local paths = load_paths()
            local ok, err = paths.validate("/mnt/verlock-data")
            assert.is_true(ok)
            assert.is_nil(err)
        end)
        ...
        it("rejects a relative path", function()
            local paths = load_paths()
            local ok = paths.validate("verlock-data")
        ...
        it("rejects a path that contains the current root", function()
            local paths = load_paths()
            local ok = paths.validate("/home/tester")
            assert.is_true(support.is_failure(ok))
        end)

        it("rejects a path inside the cache root", function()
            local paths = load_paths()
            local cache_root = paths.resolve().cache_root
            assert.is_string(cache_root)
            local ok = paths.validate(cache_root .. "/nested")
            assert.is_true(support.is_failure(ok))
        end)

        it("rejects the cache root itself", function()
            local paths = load_paths()
            local ok = paths.validate(paths.resolve().cache_root)
            assert.is_true(support.is_failure(ok))
        end)

        it("accepts a candidate that contains the cache root", function()
            support.hide_env("HOME")
            support.set_env("XDG_DATA_HOME", "/base")
            support.set_env("XDG_CACHE_HOME", "/base/steamapp-verlock")
            support.stub_millennium({ config = { data_root = "/current" } })
            local paths = load_paths()
            local ok, err = paths.validate("/base/steamapp-verlock")
            assert.is_true(ok)
            assert.is_nil(err)
        end)
        ...
        it("rejects a path that cannot be created or written", function()
            ...
            local ok = paths.validate("/mnt/locked")
            assert.is_true(support.is_failure(ok))
        end)
    end)
    ...
    describe("find_appmanifest", function()
        ...
        it("returns an error when the appmanifest is nowhere", function()
            local paths = load_paths()
            local found, err = paths.find_appmanifest("123")
            assert.is_nil(found)
